...
void musa_init(char *musa_lib_path, musa_init_resp_t *resp) {
  ...
  resp->ch.handle = LOAD_LIBRARY(musa_lib_path, RTLD_LAZY);
  if (!resp->ch.handle) {
    char *msg = LOAD_ERR();
  ...
  LOG(resp->ch.verbose, "calling muInit\n");
  ...
  LOG(resp->ch.verbose, "calling muDriverGetVersion\n");
  ...
  LOG(resp->ch.verbose, "calling muDeviceGetCount\n");
...
void musa_release(musa_handle_t h) {

...
void musart_init(char *musart_lib_path, musart_init_resp_t *resp) {
  ...
  if (!resp->ch.handle) {
    ...
    snprintf(buf, buflen,
            "Unable to load %s library to query for MUSA GPUs: %s",
            musart_lib_path, msg);
    free(msg);
    resp->err = strdup(buf);
    return;
  ...
  if (ret != MUSART_SUCCESS) {
    ...
    UNLOAD_LIBRARY(resp->ch.handle);
    resp->ch.handle = NULL;
    if (ret == MUSA_ERROR_INSUFFICIENT_DRIVER) {
      resp->err = strdup("failed to init musart, your musa driver might be too old or missing. If you have a MUSA GPU please upgrade to run ollama");
      return;
    }
